...
object etl_datos_weci_read_xsd {
  def main(args: Array[String]): Unit = {
    ...
    var df_inicial = spark.read
      ...
      //Importante esta opción permite convertir todos los campos del xml a string
      //para no tener que trabajar con otro tipo de datos
    ...
    //En esta lógica tengo todos los campos weci del DFC

    var df_final = df_inicial
      .withColumn("Worker", explode_outer(col("`Worker`")))
      .selectExpr(
        s"'$dataDatePart' as data_date_part",
        "`Worker`.`Worker_Summary`.`Employee_ID`.`_VALUE` as EMPLID",
    ...
    //En esta lógica comentada atacabamos los xml de los diferentes xsd
    //y solo devolvía vacío el xml del 19
